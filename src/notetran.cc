...
notetran::notetran(string uname) : 
	...
	no_resptime(0), ns_resptime(0),	nolo_freq(0), nohi_freq(0), nslo_freq(0), 
	nshi_freq(0), nort_samples(0), nsrt_samples(0), no_respsize(0), 
	ns_respsize(0)
{
	no_lo_notename = new char[NOTE_NAME_SIZE];
	no_hi_notename = new char[NOTE_NAME_SIZE];
	ns_lo_notename = new char[NOTE_NAME_SIZE];
	ns_hi_notename = new char[NOTE_NAME_SIZE];
	#ifndef BARE_MODULES
...
bool notetran::validate()
{
	*err_msg = "";
	if (!check_notename(no_lo_notename)) {
		*err_msg += "\n"+get_paramnames()->get_name(paramnames::PAR_NO_LONOTE) 
			+ " " + no_lo_notename;
		invalidate();
	}
	if (!check_notename(no_hi_notename)) {
		*err_msg += "\n"+get_paramnames()->get_name(paramnames::PAR_NO_HINOTE) 
			+ " " + no_hi_notename;
		invalidate();
	}
	if (!check_notename(ns_lo_notename)) {
		*err_msg += "\n"+get_paramnames()->get_name(paramnames::PAR_NS_LONOTE) 
			+ " " + ns_lo_notename;
		invalidate();
	}
	if (!check_notename(ns_hi_notename)) {
		*err_msg += "\n"+get_paramnames()->get_name(paramnames::PAR_NS_HINOTE) 
			+ " " + ns_hi_notename;
		invalidate();
	}
	if (no_resptime < 0) {
...
{
    strncpy(no_lo_notename, nol, NOTE_NAME_SIZE - 1);
    no_lo_notename[NOTE_NAME_SIZE - 1] = '\0';/* incase len > 4 chars */
}

void notetran::set_no_hi_notename(char const* noh)
{ 
    strncpy(no_hi_notename, noh, NOTE_NAME_SIZE - 1);
    no_hi_notename[NOTE_NAME_SIZE - 1] = '\0';/* incase len > 4 chars */
}
	
void notetran::set_ns_lo_notename(char const* nsl)
{ 
    strncpy(ns_lo_notename, nsl, NOTE_NAME_SIZE - 1);
    ns_lo_notename[NOTE_NAME_SIZE - 1] = '\0';/* incase len > 4 chars */
}
	
void notetran::set_ns_hi_notename(char const* nsh)
{ 
    strncpy(ns_hi_notename, nsh, NOTE_NAME_SIZE - 1);
    ns_hi_notename[NOTE_NAME_SIZE - 1] = '\0';/* incase len > 4 chars */
}
...
void notetran::create_params()
{
	...
	get_paramlist()->add_param(synthmodnames::MOD_NOTETRAN, paramnames::PAR_NO_LONOTE);
	get_paramlist()->add_param(synthmodnames::MOD_NOTETRAN, paramnames::PAR_NO_HINOTE);
	get_paramlist()->add_param(synthmodnames::MOD_NOTETRAN, paramnames::PAR_MINNO_OUT);
	get_paramlist()->add_param(synthmodnames::MOD_NOTETRAN, paramnames::PAR_MAXNO_OUT);
	get_paramlist()->add_param(synthmodnames::MOD_NOTETRAN, paramnames::PAR_DETRAN_NO);
	get_paramlist()->add_param(synthmodnames::MOD_NOTETRAN, paramnames::PAR_NO_RESPTIME);
	get_paramlist()->add_param(synthmodnames::MOD_NOTETRAN, paramnames::PAR_NS_LONOTE);
	get_paramlist()->add_param(synthmodnames::MOD_NOTETRAN, paramnames::PAR_NS_HINOTE);
	get_paramlist()->add_param(synthmodnames::MOD_NOTETRAN, paramnames::PAR_MINNS_OUT);
	get_paramlist()->add_param(synthmodnames::MOD_NOTETRAN, paramnames::PAR_MAXNS_OUT);
	get_paramlist()->add_param(synthmodnames::MOD_NOTETRAN, paramnames::PAR_DETRAN_NS);
	get_paramlist()->add_param(synthmodnames::MOD_NOTETRAN, paramnames::PAR_NS_RESPTIME);
	done_params = true;
}
#endif
#endif
