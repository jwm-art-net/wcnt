...
synthfilereader::read_synthmodule(string const *com)
{
	synthmodnames::SYNTH_MOD_TYPE 
		smt = synthmod::get_modnames()->get_type(com);
	if (smt == synthmodnames::MOD_FIRST || smt == synthmodnames::MOD_NONEZERO) 
	{
	...
	*synthfile >> modname;
	...
	// create_module() does not add to the modlist ... use add_module()
	// which is done in jwmsynth::read_and_create_synthmod(...)
	synthmod* sm = synthmod::get_modlist()->create_module(smt, modname);
	if (!read_dobjs(sm)){
...
// only reads those data objects which are defined outside of a synth module
//--------------------------------------------------------------------------
// string const* command points to the dobj name as entered by user in file.
// no need to check command is a valid dobj type name as excution would
// not have reached this far otherwise (see jwmsynth::generate_synth())
...
{	
	...
	dobjnames::DOBJ_TYPE dobjtype = dbjnames->get_type(com);
	...
	string dobjname;
	*synthfile >> dobjname;
	if (dobj::get_dobjlist()->get_dobj_by_name(&dobjname)) {
		*err_msg="\ndata object " + *com + " already exists named "+dobjname;
		return 0;
	}
	if (verbose)
...
bool synthfilereader::read_dobjs(dobj* dbj)
{
	...
	while(dd) { 
		...
		string sprogname = dbjnames->get_name(sprogtype);
		if (sprogname != com) {
			*err_msg = "\nexpected " + sprogname + " got " + com + " instead";
			delete ddlist;
			return false;
		}
		while (com == sprogname) {
			...
			if (sprog == 0) {
				*err_msg = "\n***major error***";
				*err_msg += "\ncould not create data object " + sprogname;
				*err_msg += " for data object " + *dbj->get_username();
				delete ddlist;
				return false;
			}
			if (verbose) 
			...
			if (!dbj->add_dobj(sprog)) {
				...
				*err_msg += sprogname + " to data object ";
	...
	if (com != *dbj->get_username()) {
		*err_msg = "\nexpected data object definition terminator " + 
			*dbj->get_username() + " got " + com + " instead";
		genstatus = HALTED;
		delete dbj;
		return 0;
	}
...
bool synthfilereader::read_dobjs(synthmod* sm)
{
	...
	while(mdbj) { // module may contain more than one list (ie timemap)
		...
		string xdbjname = dbjnames->get_name(dt);
		if (dbjname != xdbjname) {
			*err_msg = "\nexpected " + xdbjname + " got " 
				+ dbjname + " instead";
			delete mdbjslist;
			return false;
		}
		...
		while(dd) {//maybe the dobj has one or more dobjies inside?
			dobjnames::DOBJ_TYPE sprogtype = dd->get_dobj_sprog();
			string xsprogname = dbjnames->get_name(sprogtype);
			string com = *read_command();
			// now read the list of items ( each item's type is sprogtype )
			while (com != dbjname) {
				if (com != xsprogname) { // check name of item matches expected
					*err_msg = "\nexpected " + xsprogname + " got "
						+ com + " instead, in data object " + xdbjname;
					delete mdbjslist;
					return false;
				}
				if (verbose)
				...
				if (dbj == 0) { // failed to create dobj
					*err_msg = 
						...
						+ xsprogname + ", in data object " + xdbjname;
					delete mdbjslist;
					return false;
				}
				if (!read_dobj_params(dbj)) {
					*err_msg += ", in data object " + xdbjname;
					delete mdbjslist;
					return false;
				}
...
bool synthfilereader::read_dobj_params(dobj* dbj)
{
	...
	dobjparamlist* dparlist = dobj::get_dobjparamlist()->
		get_dobjparamlist_for_dobj_type(dobjtype);
	dobjparam* dparam = dparlist->goto_first();
	dparnames* dpnames = dobj::get_dparnames();
	while(dparam) {
		string dparname = *read_command();
		if (dpnames->get_name(dparam->get_dpartype()) != dparname) {
			*err_msg = "\nexpected parameter name " +
				dpnames->get_name(dparam->get_dpartype()) + 
				" got " + dparname + " instead, in data object "
				+ dobj::get_dobjnames()->get_name(dobjtype);
			delete dparlist;
			return false;
		}
...
bool synthfilereader::read_inputs(synthmod* sm)
{
	...
	while(inp) { // step through each  input for module
		...
		if (innames->get_name(inp->get_inputtype()) != inputname){
			*err_msg = "\nExpected input type " 
				+ innames->get_name(inp->get_inputtype()) + " for module " 
				+ *sm->get_username() + " got " + inputname + " instead.";
			delete inlist;
			return false;
		} else {
			...
			*synthfile >> outmodname;
			if (outmodname == "off") {
				outputnames::OUT_TYPE offout = 
				  outnames->get_nonezerotype(innames->get_category(input_type));
				connector* connect = 
					new connector(sm, input_type, outmodname, offout);
			...
			else
			{
				...
				*synthfile >> outputname;
				outputnames::OUT_TYPE output_type = 
					outnames->get_type(&outputname);
				if (output_type == outputnames::OUT_FIRST) {
					*err_msg = "\nInput " + inputname + " in module " +
						*sm->get_username() + " set with unknown output type " 
						+ outputname;
					delete inlist;
					return false;
				}
					...
					!= outnames->get_category(output_type))
				{
					*err_msg = "\nInput " + inputname + " in module " 
						+ *sm->get_username() 
						+ " set with output type of wrong category " 
						+ outputname;
					delete inlist;
					return false;
				}
				connector* connect = new 
					connector(sm, input_type, outmodname, output_type);
				synthmod::get_connectlist()->add_connector(connect);
				if (verbose){
...
bool synthfilereader::read_params(synthmod* sm)
{
	modparamlist* parlist = synthmod::get_paramlist()
		->get_paramlist_for_moduletype(sm->get_module_type());
	if (!parlist) {
		*err_msg = "\nproblems getting params for module "
			+ *sm->get_username();
		return false;
	}
	...
	while(param) {
		...
		if (parnames->get_name(param->get_paramtype()) != paramname) {
			*err_msg = "\nexpected parameter " 
				+ parnames->get_name(param->get_paramtype()) + " for module "
				+ *sm->get_username() + " got " + paramname + " instead";
			delete parlist;
			return false;
		} else {
			...
			if (conv) delete conv;
			conv = new ostringstream;
			IOCAT iocat = synthmod::get_paramnames()->get_category(paramtype);
			void* data = read_iocat_value(iocat);
			if (!data) {
				*err_msg += " in module " + *sm->get_username();
				genstatus = HALTED;
			...
			if (!sm->set_param(paramtype, data)){
				destroy_iocat_data(iocat, data);
				*err_msg = "\nmodule " + *sm->get_username() 
					+ " denied owning parmeter of type " + paramname;
				genstatus = HALTED;
				return false;
			}
			destroy_iocat_data(iocat, data);
	...
	switch(iocat)
	{
		...
		case CAT_ULONG:
			...
			if (!(*synthfile >> *(unsigned long*)data)) {
				*err_msg = "\nexpected unsigned long integer value";
		...
		case CAT_STATE:// except in behaviour
			...
			*synthfile >> svalue;
			if (svalue == "on")
				...
				*err_msg = "\nexpected string value on or off";
				delete (STATUS*)data;
				return 0;
			}
			*conv << svalue;
		...
		case CAT_METER:
			...
			if (!(*synthfile >> ((timesig*)data)->beatsperbar)) {
				*err_msg = "\nexpected integer value for time signature ";
				*err_msg += "beats per bar";
				delete (timesig*)data;
				return 0;
			}
			char ch;
			...
			if (ch != '/') {
				*err_msg = "\nexpected seperator / for top and bottom halves";
				*err_msg += " of time signature";
				delete (timesig*)data;
				return 0;
			}
		...
		case CAT_ADSRSECT:
			...
			*synthfile >> svalue;
			if (svalue == "attack") 
				...
				*err_msg = "\nunrecognised ADSR section " + svalue;
		...
		case CAT_DOBJ:
			if (!(*synthfile >> svalue)) {
				*err_msg = "\nunable to read name of data object";
				return 0;
			}
			data = dobj::get_dobjlist()->get_dobj_by_name(&svalue);
			if (!data) {
				*err_msg = "\nno data object named " + svalue + " found";
				return 0;
			}
			*conv << svalue;
			break;
		case CAT_SYNTHMOD:
			if (!(*synthfile >> svalue)) {
				*err_msg = "\nunable to read synth module name";
				return 0;
			}
			data = synthmod::get_modlist()->get_synthmod_by_name(&svalue);
			if (!data) {
				*err_msg = "\nno synth module found named " + svalue;
				return 0;
			}
			*conv << svalue;
			break;
