...
connector::connector(synthmod* input_module, 
...
bool connector::connect()
{
	...
	synthmod* outmod = synthmod::get_modlist()->get_synthmod_by_name(&out_mod_uname);
	if (!outmod) {
		connect_err_msg = 
			"\nCannot make connection, output module not found: " + out_mod_uname
		...
		synthmod::get_outputnames()->getcategory(out_type)) 
	{
		connect_err_msg = 
			...
			+ *in_mod->get_username() + synthmod::get_inputnames()->getname(in_type)
			+ "\noutput: " + out_mod_uname + synthmod::get_outputnames()->getname(out_type);
		return false;
	}
	if (!in_mod->set_in(in_type, out_data)) {
		connect_err_msg = 
			"\nCannot make connection, input type: " + synthmod::get_inputnames()->getname(in_type)
