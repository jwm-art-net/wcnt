...
bool dynamic::validate()
{
	if (!goto_first()) {
		*err_msg = "\n***oops*** try adding some dvertex to amp_map";
		*err_msg += "\nin module " + *get_username();
		invalidate();
	} else if (!goto_next()) {
...
dynvertex* dynamic::add_dvertex(double sil, double usol, double lsol)
{
	...
	if (!ordered_insert(dynamics, dvtx1, &dynvertex::get_signal_in_level)){
		delete dvtx1;
		return 0;
	}
	return dvtx1;
...
void dynamic::create_params()
{
	if (done_params == true)
		return;
	get_paramlist()->
			add_param(synthmodnames::MOD_DYNAMIC, paramnames::PAR_UP_THRESH);
	get_paramlist()->
			add_param(synthmodnames::MOD_DYNAMIC, paramnames::PAR_LO_THRESH);
	get_paramlist()->
			add_param(synthmodnames::MOD_DYNAMIC, paramnames::PAR_POSNEG_MIRROR);
	get_paramlist()->
			add_param(synthmodnames::MOD_DYNAMIC, paramnames::PAR_USE_RATIOS);
	done_params = true;
}
...
void dynamic::create_dobj()
{
	...
	dobj::get_dobjdobjlist()->
		add_dobjdobj(dobjnames::LIN_DYNAMICS, dobjnames::SIN_DVERTEX);
	done_dobj = true; 
}
#endif
#endif
