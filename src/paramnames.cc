...
const paramnames::param_data paramnames::data[LAST] =
{
    ...
    { STEP_COUNT,       "step_count",           iocat::SHORT,   "Number of steps to split allowed output values into." },
    ...
    { OCTAVE,           "octave_offset",        iocat::SHORT,   "Number of octaves to adjust frequency." },
    ...
    { START_POS_MIN,    "start_pos_min",        iocat::ULONG,   "Minimum start position (samples)." },
    { START_POS_MAX,    "start_pos_max",        iocat::ULONG,   "Maximum start position (samples)." },
    { LOOP_BEGIN,       "loop_start_pos",       iocat::ULONG,   "Start position of loop (samples)." },
    { LOOP_END,         "loop_end_pos",         iocat::ULONG,   "End of loop position (samples)." },
    { LOOP_MODE,        "loop_mode",            iocat::FIX_STR, "Loop mode." },
    { ANTI_CLIP,        "anti_clip_samples",    iocat::SHORT,   "Number of samples to x-fade when playback starts or looping occurs." },
    ...
    { LOOP_BI_OFFSET,   "bi-directional_offset",iocat::SHORT,   "Number of samples to offset when reaching loop points of bi-directional loop." },
    ...
    { START_BAR,        "start_bar",            iocat::SHORT,   "Bar number (0 = first bar) to start processing on." },
    { END_BAR,          "end_bar",              iocat::SHORT,   "Bar number (0 = first bar) to end processing on." },
    { ZERO_SEARCH_RANGE,"zero_search_range",    iocat::SHORT,   "Number of samples to search around play/loop points for zero-crossing. " },
    ...
    { EXIT_BAR,         "exit_bar",             iocat::SHORT,   "Bar to stop all processing and exit the program at." },
    ...
    { QUARTER_VAL,      "quarter_value",        iocat::SHORT,   "Ticks Per Quarter Note. A value divisible by both 3 and 4 is a good start." },
    { RELEASE_RATIO,    "release_is_ratio",     iocat::STATE,   "Whether to treat release level as a ration when release stage activates." },
    ...
    { PRECISION,        "precision",            iocat::SHORT,   "Number of samples apart the two inputs may trigger to still be considered simultaneous." },
    ...
    { BAR,              "bar",                  iocat::SHORT,   "Bar number (starting at 0)." },
    ...
    { TRANSPOSE,        "transpose",            iocat::SHORT,   "Number of semitones to transpose the notes (none-fractional)." },
    ...
    { MAXPEAKS,         "max_peaks",            iocat::SHORT,   "Maximum number of peaks that may be detected before message display stopped or forced abort." },
    { REPEAT,           "repeat",               iocat::SHORT,   "How many times to repeat the riff along bars in the timeline." },
    { REPEAT_STRIPE,    "repeat_stripe",        iocat::SHORT,   "Repeat every n bars." },
    ...
    { TEST_ITER,        "test_iterations",      iocat::ULONG,   "Number of fractal iterations to perform to obtain an output scaling ratio in attempt to limit output within -1.0 ~ +1.0 range." },
    ...
    { STAGES,           "stages",               iocat::SHORT,   "Number of filter stages." },
    ...
    /* inspect module's bs */
    { INSPECT_DF_MSG,   "_double_float_msg_",   iocat::STRING,  "Message to display when float value is triggered."  },
    { INSPECT_SI_MSG,   "_short_int_msg_",      iocat::STRING,  "Message to display when short int value triggered."  },
    { INSPECT_UL_MSG,   "_long_int_msg_",       iocat::STRING,  "Message to display when long int value triggered."  },
    { INSPECT_ST_MSG,   "_state_msg_",          iocat::STRING,  "Message to display when state value triggered."  },
    { INSPECT_STR_MSG,  "_string_msg_",         iocat::STRING,  "Message to display when string value triggered."  },
    { PRE_COUNT,        "pre_count",            iocat::SHORT,   "How many triggers to ignore before counting." },
