...
{ // do a whole load of checks on the users input.
	if (min_start_pos > max_start_pos) {
		int mxsp = max_start_pos;
		max_start_pos = min_start_pos;
		min_start_pos = mxsp;
	}
	int halfsr = search_range / 2;
	if (ac_each_end == ON) {
		wavstart = zero_search(anti_clip_size + halfsr, search_range);
		wavlength = zero_search(wavlength-anti_clip_size-halfsr,search_range);
	} else {
...
// I was eating a platefull of spaghetti in tomata sauce, on 
// toast.  I'd eaten several mouthfulls when I noticed a rather
// unpleasant looking armoured worm slowly writhing in my meal.
// it was around 20 cm long and had three feelers on it's head,
// So i stabbed it with my fork and threw it on the floor, where
// it promptly rushed at me trying to find it's way into my
// stomache, the f***ing little bastard.
// Note: 
// that was a couple of weeks ago and at that time a lot of the code
// was inside sampler::run(), since then it's been taken out and put
// into private methods, but has continued expanding...
...
void sampler::run()
{
	...
	if (play_state == ON) {
		...
		cp_ratio = *in_deg_size / root_deg_size;//sample rate ratio is sr_ratio
		...
		if (ch == WAV_CH_MONO) {
			...
			if (do_ac == ON)  {
				...
				out_left = (short)(out_left * ac_size + ac_out_left 
					* (1 - ac_size));
			}
			out_right = out_left;
...
void sampler::trigger_playback()
{
	...
	if (playdir == PLAY_FWD) {
		...
		else buffer_start_pos =(unsigned long)(cur_pos - (WAV_BUFFER_SIZE -2));
	}
	fill_buffer(buffer_start_pos);
	if (loop_mode == LOOP_OFF)
		loop_yet = 0; // <-- why the farmer wore two pairs of trouser braces.
	else {
...
unsigned long sampler::zero_search(unsigned long pos, short range)
{
	...
	if (ch == WAV_CH_MONO) {
		...
		for (int i = 0; i < halfsr; i++) {
			int si = range - i;
			int s1=((sst_buf[i].left>0)?sst_buf[i].left:-sst_buf[i].left);
			s1 += ((sst_buf[i].right>0)?sst_buf[i].right:-sst_buf[i].right);
			int s2=((sst_buf[si].left>0)?sst_buf[si].left:-sst_buf[si].left);
			s2+=((sst_buf[si].right>0)?sst_buf[si].right:-sst_buf[si].right);
			if (smallest1 > s1) {
				smallest1 = s1;
				smallest_pos1 = buf_st_pos + i;
			} 
...
// active, there are four of them.  ac_copy_fwd... is called by anti_clip_fwd
	...
	if (acplaydir == PLAY_FWD) {	
		for (int i = 0; i <= anti_clip_size; i++) {
			if (ac_buf_pos + i <= anti_clip_size && tmp_acsz <= 1) {
				ac_m_tmp[i] = (short)(ac_m_buf[ac_buf_pos + i]*(1 - tmp_acsz));
				tmp_acsz += ac_step;
			}
			else ac_m_tmp[i] = 0;
		}
...
void sampler::ac_copy_fwd_stereo(stereodata* ac_st_tmp) 
{
	...
	if (acplaydir == PLAY_FWD) {
		for (int i = 0; i <= anti_clip_size; i++) {
			if (ac_buf_pos + i <= anti_clip_size && tmp_acsz <= 1) {
				ac_st_tmp[i].left = (short)
					(ac_st_buf[ac_buf_pos + i].left	* (1 - tmp_acsz));
				ac_st_tmp[i].right = (short)
...
void sampler::ac_mix_fwd_stereo(stereodata* ac_st_tmp)
{
	int i = 0;
	while (ac_size < 1) {
		ac_st_buf[i].left = (short)
			(ac_st_buf[i].left * ac_size + ac_st_tmp[i].left);
		ac_st_buf[i].right = (short)
			(ac_st_buf[i].right * ac_size + ac_st_tmp[i].right);
		ac_size += ac_step;
		i++;
	}
}
	...
	if (acplaydir == PLAY_FWD) {
		for (int i = 0; i <= anti_clip_size; i++) {
			if (ac_buf_pos + i <= anti_clip_size && tmp_acsz <= 1) {
				...
				= (short)(ac_st_buf[ac_buf_pos + i].left * (1 - tmp_acsz));
				ac_st_tmp[anti_clip_size - i].right
				= (short)(ac_st_buf[ac_buf_pos + i].right * (1 - tmp_acsz));
				tmp_acsz += ac_step;
			} else {
...
void sampler::ac_mix_rev_mono(short* ac_m_tmp)
{
	...
	while (ac_size <= 1 && i >= 0) {
		ac_m_buf[i] = (short)(ac_m_buf[i] * ac_size
			+ ac_m_tmp[i]);
		ac_size += ac_step;
		i--;
	}
