...
nonezero::nonezero(const char* uname) :
 ...
 out_none_double(0.0), out_one_double(1.0), out_none_wcint_t(0),
 out_none_samp_t(0), out_none_STATUS(OFF), out_none_string(0)
{
    out_none_string = new char[1];
    *out_none_string = '\0';
    register_output(output::OUT_NONE_DOUBLE);
    register_output(output::OUT_ONE_DOUBLE);
...
const void* nonezero::get_out(output::TYPE ot) const
{
    switch(ot)
    {
        ...
        case output::OUT_ONE_DOUBLE:   return &out_one_double;
