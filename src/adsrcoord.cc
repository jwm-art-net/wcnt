...
void adsr_coord::register_ui()
{
    ...
    register_param(param::UPTIME)->set_flags(ui::UI_CHOICE1);
    register_param(param::UPLEVEL)->set_flags(ui::UI_CHOICE1);
    register_param(param::LOTIME)->set_flags(ui::UI_CHOICE1);
    register_param(param::LOLEVEL)->set_flags(ui::UI_CHOICE1);
    register_comment(" - OR - ");
    register_param(param::TIME, "Specify section time for both upper and lower shapes.")
                                    ->add_descr("section time")
                                    ->set_flags(ui::UI_CHOICE2);
