...
modifier::modifier(string uname) 
: synthmod(synthmodnames::MOD_MODIFIER, modifier_count, uname),
  in_signal(0), in_mod(0), out_output(0.00), modifier_func(ADD), mod_size(1.00), 
  min_out_val(0.00), max_out_val(1.00), posnegmirror(OFF), type_func(0)
{
	...
	if (!get_inputlist()->add_input(this, inputnames::IN_MODIFIER)){
		invalidate();
		return;
	}
	modifier_count++;
...
void modifier::create_params()
{
	if (done_params == true)
		return;
	get_paramlist()->add_param(synthmodnames::MOD_MODIFIER, paramnames::PAR_MODIFIER_FUNC);
	get_paramlist()->add_param(synthmodnames::MOD_MODIFIER, paramnames::PAR_MODIFIER_SIZE);
	get_paramlist()->add_param(synthmodnames::MOD_MODIFIER, paramnames::PAR_MIN_OUT);
	get_paramlist()->add_param(synthmodnames::MOD_MODIFIER, paramnames::PAR_MAX_OUT);
	get_paramlist()->add_param(synthmodnames::MOD_MODIFIER, paramnames::PAR_POSNEG_MIRROR);
	done_params = true;
}
