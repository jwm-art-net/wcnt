...
//	specified in the notetran parameters, it is translated to a value within
//	the output range.  if the note is outside of the input range, the output
//	remains unchanged.

// the triggers triggering depends on note_on or note_slide causing the
// translation, and if the note name is within or out side of the corresponding
// note range.

class notetran: public synthmod 
{
 public:
	notetran(string uname);
	~notetran();
	// inputs
	...
	void set_input_note_slide_trig(STATUS const* st){ in_note_slide_trig = st;}
	...
	STATUS const* get_output_note_slide_trig(){ return &out_note_slide_trig;}
