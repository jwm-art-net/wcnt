...
/*
    ...
    the list object's destructor will pass through the list and delete
    each listed object pointed to by ll_item::data but it will not delete
    ll_item. That job should be researved for the linkedlist destructor.

    NULL CHECKING
    -------------
    I've added checking for this==NULL to ll_item::get_????() functions,
    but not to ll_item::set_????() functions.  This is because I thought
    it better to get segfaults from trying use the set_????() functions on
    a NULL ll_item than not doing anything. In any case the user should
    check, and linkedlist code refuses to add a NULL ll_item to the list
    anyway.

    Checking for this==NULL in the get_????() functions is a whole lot
    more usefull as it saves extra checks elsewhere.
    ...
    ------------------------
    add_at_head(ll_item*), and add_at_tail(ll_item*) return the ll_item
    added if nothing went wrong.  insert_item_after(...) returns NULL if
    the after item does not exist in the list, else the ll_item added.

    unlink item returns null if the item does not exist in the list, else
    the item unlinked.

    goto_????() functions set current to, and return NULL if something
    goes wrong. sneak_????() functions just return next or prev without
    ...
    -------------------------------------------------------------------
    this can be used with any object type which has a method which returns
    any type of value that can be used in comparisons.  Use this to check
    for a data item value within an object in a linkedlist.  to search
    again use lookup_data_match_next.  
    NOTE: the lookup... template functions use set/get tempory so do not
    change tempory in between lookups if you want them to work properly.
    see far below.
*/
...
/*
    ...
    to use to get a numerical value from the object pointed to by the
    ll_items in the list.  the function must return a value that can be
    compared. this assumes that all ll_items in the list are all of the
    same class the pointed-to function comes from.
*/
...
/*
    LOOKUP_DATA_MATCH
    --------------------------------------
    pass the linkedlist to search through (thelist), the data object
    which contains the value to check for (data), and the function which
    returns that value (match_func).
*/
...
/*
    ...
    -------------------------------------------------
    more or less the same, but pass the data object returned from
    lookup_data_match ie an object already in the list to start the 
    search from. (instead of an object outside of the list).
*/
