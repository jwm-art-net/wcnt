...
#define SR_DIV_BI 8
// SR_DIV_BI - divide search range by n.  tells the sampler how many samples to 
// search for a match closest to zero when calculating the bi-directional 
// offset from the current position when changing direction.
// result should be pretty small because the user specifies the offset from the 
// loop point.  The target is to jump half a phase in the waveform so that 
// continuity of alternating positive and negative waveforms is not lost.
...
// loop_dir - start loop with loop mode if loop_bi continue with current dir.
